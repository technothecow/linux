...
void mmc_request_done(struct mmc_host *host, struct mmc_request *mrq)
{
	struct mmc_command *cmd = mrq->cmd;
	int err = cmd->error;
	printk(KERN_INFO "mmc_request_done 1, err = %d", err);
	 ...
	 */
	printk(KERN_INFO "mmc_request_done 6");
	if (!err || !cmd->retries || mmc_card_removed(host->card)) {
		printk(KERN_INFO "mmc_request_done 6.1");
		mmc_should_fail_request(host, mrq);

		printk(KERN_INFO "mmc_request_done 6.2");
		if (!host->ongoing_mrq)
			led_trigger_event(host->led, LED_OFF);

		printk(KERN_INFO "mmc_request_done 6.3");
		if (mrq->sbc) {
		...
		printk(KERN_INFO "mmc_request_done 6.4");
		pr_debug("%s: req done (CMD%u): %d: %08x %08x %08x %08x\n",
			mmc_hostname(host), cmd->opcode, err,
			cmd->resp[0], cmd->resp[1],
			cmd->resp[2], cmd->resp[3]);
		printk(KERN_INFO "mmc_request_done 6.5");
		...
		printk(KERN_INFO "mmc_request_done 6.6");
		...
		printk(KERN_INFO "mmc_request_done 6.7");
	}
	/*
	...
	printk(KERN_INFO "mmc_request_done 7");
	if (mrq->done)
		mrq->done(mrq);
	printk(KERN_INFO "mmc_request_done 8");
}
